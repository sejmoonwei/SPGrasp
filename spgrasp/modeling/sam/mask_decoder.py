...
from spgrasp.modeling.spgrasp_utils import LayerNorm2d, MLP
...
class MaskDecoder(nn.Module):
    ...
    def forward(
        ...
        #OCID 20,5,128,128  20,5  20,5,26  20,1  graspnet:3,1,128,128  3,1  3,1,256  3,1  graspnetnew 3,5,128,128 3,5 3,5,256 3,1
    def predict_masks(
        self,
        image_embeddings: torch.Tensor,
        image_pe: torch.Tensor,
        ...
        tokens = torch.cat((output_tokens, sparse_prompt_embeddings), dim=1)   #output_tokens 5,124,256  sparse_prompt_embeddings 5,3,256
        #5,127,256
        ...
        src = src + dense_prompt_embeddings #5,256,32,32
        assert (
            image_pe.size(0) == 1
        ...
        # Run the transformer
        hs, src = self.transformer(src, pos_src, tokens) #5,127,256
        iou_token_out = hs[:, s, :] #5,256
        mask_tokens_out = hs[:, s + 1 : (s + 1 + self.num_mask_tokens), :] #5,123,256
        ...
        masks = (hyper_in @ upscaled_embedding.view(b, c, h * w)).view(b, -1, h, w) #5,123,32  5,32,128,128
        #5,125,128,128
        ...
        return masks, iou_pred, mask_tokens_out, object_score_logits #5,123,128,128  5,123    5,123,256        5,1
