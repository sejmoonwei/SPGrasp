...
from spgrasp.modeling.spgrasp_base import SAM2Base
from spgrasp.modeling.spgrasp_utils import (
    get_1d_sine_pe,
    get_next_point,
    sample_box_points,
...
from spgrasp.utils.misc import concat_points

from training.utils.data_utils import BatchedVideoDatapoint


class SPGraspTrain(SAM2Base):
    def __init__(
        self,
        image_encoder,
    ...
    def forward_tracking(
        ...
        for stage_id in processing_order: # for frame in list
            # Get the image features for the current frames
            # img_ids = input.find_inputs[stage_id].img_ids
            img_ids = input.flat_obj_to_img_idx[stage_id]
            if img_feats_already_computed:
                # Retrieve image features according to img_ids (if they are already computed).
                current_vision_feats = [x[:, img_ids] for x in vision_feats] #flattened
    ...
    def track_step(
        ...
        current_out["multistep_point_inputs"] = [point_inputs] #need fix?
    ...
    def _iter_correct_pt_sampling(
        ...
        for _ in range(self.num_correction_pt_per_frame): # Sample 4 times to get 4 sets of inputs/outputs
            ...
            if self.mode == 'sam':
                multimask_output = self._use_multimask(is_init_cond_frame, point_inputs) #fix
            else:
