...
def single_node_runner(cfg, main_port: int):
    ...
    if num_proc == 1:
        ...
        single_proc_run(local_rank=0, main_port=main_port, cfg=cfg, world_size=num_proc) # gpu device
    else:
...
def main(args) -> None:
    cfg = compose(config_name=args.config) #sam2.1_hiera_b+_MOSE_finetune.yaml
    if cfg.launcher.experiment_log_dir is None:
        cfg.launcher.experiment_log_dir = os.path.join(
            os.getcwd(), "spgrasp_logs", args.config
        )
...
if __name__ == "__main__":
    ...
    initialize_config_module("spgrasp", version_base="1.2")
    ...
    parser.add_argument(
        ...
        required=True,
        type=str,
        help="path to config file (e.g. spgrasp/configs/sam2.1_training/sam2.1_hiera_b+_MOSE_finetune.yaml)",
    )
    parser.add_argument(
        "--use-cluster",
        type=int,
