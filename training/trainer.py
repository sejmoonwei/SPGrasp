...
class Trainer:
    ...
    def _load_resuming_checkpoint(self, ckpt_path: str):
        ...
        logging.info("--- Optimizer State Dict Mismatch Diagnosis ---")
        try:
            current_optimizer_pg = self.optim.optimizer.param_groups
            checkpoint_optimizer_pg = checkpoint["optimizer"]['param_groups']
            logging.info(f"Number of param groups in current optimizer: {len(current_optimizer_pg)}")
            logging.info(f"Number of param groups in checkpoint: {len(checkpoint_optimizer_pg)}")

            for i, group in enumerate(current_optimizer_pg):
                logging.info(f"  Current optimizer - Group {i}: {len(group['params'])} params.")

            for i, group in enumerate(checkpoint_optimizer_pg):
                logging.info(f"  Checkpoint optimizer - Group {i}: {len(group['params'])} params.")
        except Exception as e:
            logging.error(f"Error during optimizer diagnosis: {e}")
        logging.info("--- End of Diagnosis ---")
    ...
    def visualize_5d_tensor(tensor, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]):
        ...
        plt.imshow(img_np)
        plt.axis('off')
        plt.title("Normalized Image Visualization")
        plt.show()
    ...
    def visualize_grasp_labels(label_tensor, object_idx=0):
        ...
        for idx in range(5):
            ...
            im = ax.imshow(channel_data,
                           ...
                           vmin=channels[idx]["vmin"],
                           vmax=channels[idx]["vmax"],
    ...
    def _step(
        ...
        outputs = model(batch) #  input to model  list[x,x,x,x]  each sample contains a dict
        targets = batch.masks #bs1: 1,1,512,512

        #          vis              #
        # batch.img_batch #1,1,3,512,512
        # batch.masks #1,4,5,512,512
        # self.visualize_5d_tensor(batch.img_batch)
        # self.visualize_grasp_labels(targets)
        #                            #
    ...
    def _setup_dataloaders(self):
        ...
        if self.mode in ["train", "train_only"]:
            self.train_dataset = instantiate(self.data_conf.train) #torchtrainMixeddataset
    ...
    def train_epoch(self, train_loader):
        ...
        for data_iter, batch in enumerate(train_loader): #batch:batchedVideoDatapoint
            # measure data loading time
