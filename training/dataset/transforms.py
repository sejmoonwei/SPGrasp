...
class RandomResizeAPI:
    ...
    def __call__(self, datapoint, **kwargs):
        if self.consistent_transform:
            ...
            for i in range(len(datapoint.frames)):
                if len(datapoint.frames[0].objects[0].segment.shape) == 3: #fix OCID
                    datapoint = resize_grasp(datapoint, i, size, self.max_size, square=self.square, v2=self.v2)
                    # pass
                elif len(datapoint.frames[0].objects[0].segment.shape) == 2:
                    datapoint = resize(
