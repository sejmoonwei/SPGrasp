...
from yolox.data.data_prefetcher import GraspDataPrefetcher
...
class Trainer:
    ...
    def after_epoch(self):
        ...
        # 按间隔保存epoch checkpoint
        ckpt_interval = getattr(self.exp, 'ckpt_interval', 0)
        if ckpt_interval > 0 and (self.epoch + 1) % ckpt_interval == 0:
...
class GraspTrainer(Trainer):
    """
    Trainer for ByteTrack-Grasp that handles grasp GT.
    """
    ...
    def before_train(self):
        ...
        logger.info("init GraspDataPrefetcher, this might take one minute or less...")
        # 使用 GraspDataPrefetcher 处理抓取GT
        self.prefetcher = GraspDataPrefetcher(self.train_loader)
        # max_iter means iters per epoch
        self.max_iter = len(self.train_loader)
    ...
    def train_one_iter(self):
        ...
        # GraspDataPrefetcher 返回 (inps, targets, grasp_targets)
        inps, targets, grasp_targets = self.prefetcher.next()
        ...
        if grasp_targets is not None:
            grasp_targets = {k: v.to(self.data_type) for k, v in grasp_targets.items()}

        data_end_time = time.time()

        with torch.cuda.amp.autocast(enabled=self.amp_training):
            outputs = self.model(inps, targets, grasp_targets)
        loss = outputs["total_loss"]
